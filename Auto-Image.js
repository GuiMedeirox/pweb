(async () => {
  ...
  const state = {
    ...
    estimatedTime: 0,
    language: 'en',
    paintedPixelsMap: new Set() // Adicionado para cache de pixels pintados
  };
  ...
  // Função para obter a cor atual de um pixel no canvas do jogo
  async function getCurrentPixelColor(regionX, regionY, pixelX, pixelY) {
    try {
      // Aqui você precisaria fazer uma requisição GET para obter o estado atual
      // da região e verificar a cor do pixel específico
      const response = await fetch(`https://backend.wplace.live/s0/region/${regionX}/${regionY}`);
      const regionData = await response.json();
      
      // Extrair a cor do pixel específico dos dados da região
      // (a estrutura exata dependeria da API do wplace)
      return getPixelColorFromRegionData(regionData, pixelX, pixelY);
    } catch {
      return null; // Se falhar, assume que precisa pintar
    }
  }
  ...
  async function processImage() {
    ...
    for (let y = startRow; y < height; y++) {
      for (let x = (y === startRow ? startCol : 0); x < width; x++) {
        ...
        // NOVA VERIFICAÇÃO: Pular se já foi pintado
        if (state.paintedPixelsMap.has(pixelKey)) {
          continue;
        }
        
        // OPCIONAL: Verificar cor atual no canvas (mais preciso mas mais lento)
        const currentColor = await getCurrentPixelColor(regionX, regionY, pixelX, pixelY);
        const targetColor = findClosestColor([r, g, b], state.availableColors);
        
        if (currentColor === targetColor) {
          state.paintedPixelsMap.add(pixelKey);
          continue; // Pixel já está correto
        }
  ...
  function saveProgress() {
    const progressData = {
      paintedPixelsMap: Array.from(state.paintedPixelsMap),
  ...
  function loadProgress() {
    ...
    if (saved) {
      ...
      state.paintedPixelsMap = new Set(data.paintedPixelsMap);
